...
abstract class TableTemplet {
     ...
     * 行样式
     */
    public function rowClass($data) { return ''; }
    ...
    public function view($custom_config = []) {
        ...
        echo '<table class="table table-striped table-bordered table-hover ' . $this->align . '" style="white-space: pre;">';
        //thead
        echo '<thead><tr>';
        $No = false;
        if (isset($config['No'])) {
        ...
        foreach($viewField as $v) {
            echo '<th class="' . $this->align . '">', $v['title'] , '</th>';
        }
        echo '</tr></thead>';
        //tbody
